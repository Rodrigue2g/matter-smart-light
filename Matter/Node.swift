...
struct MatterOnOffLight: MatterConreteEndpoint {
  static var deviceTypeId: UInt32 { esp_matter.endpoint.on_off_light.get_device_type_id() }

  var endpoint: UnsafeMutablePointer<esp_matter.endpoint_t>

  init(_ node: RootNode, configuration: esp_matter.endpoint.on_off_light.config_t) {
    var config = configuration
    endpoint = esp_matter.endpoint.on_off_light.create(node.node, &config, 0x00, Unmanaged.passRetained(node.context).toOpaque())
  }
